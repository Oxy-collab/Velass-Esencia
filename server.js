...
app.post('/api/transactions', (req, res) => {
    ...
    db.run(
        ...
        function (err) {
            if (err) return res.status(500).json({ error: err.message });
            getOne('SELECT * FROM transactions WHERE id = ?', [this.lastID], (err, transaction) => {
                if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/transactions', (req, res) => {
    getAll('SELECT * FROM transactions ORDER BY date DESC, created_at DESC', [], (err, transactions) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.get('/api/transactions/:month', (req, res) => {
    ...
    getAll('SELECT * FROM transactions WHERE date LIKE ? ORDER BY date DESC', [`${month}%`], (err, transactions) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(transactions);
    });
});
